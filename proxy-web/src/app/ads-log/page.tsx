...
export default function AdsLogPage() {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      eventSource = new EventSource("http://localhost:3000/api/ads-logs/stream");
  ...
  // 초기 로그 로드
  useEffect(() => {
    fetch("http://localhost:3000/api/ads-logs?limit=100")
      .then((res) => res.json())
      .then((data) => {
        setLogs(data.logs || []);
      })
  ...
  const handleClearLogs = async () => {
    try {
      await fetch("http://localhost:3000/api/ads-logs", { method: "DELETE" });
      setLogs([]);
    } catch (e) {
      console.error("Failed to clear ads logs:", e);
    }
  };
    ...
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-4">
          <h1 className="text-2xl font-bold">Ads Log</h1>
          <span
          ...
          {virtualItems.items.map(({ log, style }) => (
            <div key={log.id} style={style} className="px-2 py-1">
              <div
                ...
                onClick={() => router.push(`/ads-log/${log.id}`)}
              >
